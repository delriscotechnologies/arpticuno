...
def measure_coverage(root: Path) -> tuple[int, int]:
    covered = 0
    executable = 0

    for report in sorted(root.rglob("*.cover")):
    ...
    if executable == 0:
        raise ValueError(f"No executable Python lines were found under {root}")
...
def main() -> int:
    ...
    covered, executable = measure_coverage(args.report_directory)
